...
import com.icounseling.service.CommentService;
...
/**
 * REST controller for managing {@link com.icounseling.domain.Comment}.
 */
@RestController
@RequestMapping("/api")
public class CommentResource {
     ...
     */
    @GetMapping("/comments")
    public ResponseEntity<List<CommentDTO>> getAllComments(Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder, @RequestParam(required = false) String filter) {
        if ("document-is-null".equals(filter)) {
            log.debug("REST request to get all Comments where document is null");
            return new ResponseEntity<>(commentService.findAllWhereDocumentIsNull(),
                    HttpStatus.OK);
        }
