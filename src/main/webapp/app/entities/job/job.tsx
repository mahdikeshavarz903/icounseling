...
export class Job extends React.Component<IJobProps, IJobState> {
  ...
  render() {
    ...
    return (
      <div>
        <h2 id="job-heading">
          <Translate contentKey="iCounselingApp.job.home.title">Jobs</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="iCounselingApp.job.home.createLabel">Create new Job</Translate>
          </Link>
        </h2>
            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={this.sort('jobTitle')}>
                  <Translate contentKey="iCounselingApp.job.jobTitle">Job Title</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={this.sort('minSalary')}>
                  <Translate contentKey="iCounselingApp.job.minSalary">Min Salary</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={this.sort('maxSalary')}>
                  <Translate contentKey="iCounselingApp.job.maxSalary">Max Salary</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="iCounselingApp.job.visitor">Visitor</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {jobList.map((job, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{job.minSalary}</td>
                  <td>{job.maxSalary}</td>
                  <td>{job.visitorId ? <Link to={`visitor/${job.visitorId}`}>{job.visitorId}</Link> : ''}</td>
